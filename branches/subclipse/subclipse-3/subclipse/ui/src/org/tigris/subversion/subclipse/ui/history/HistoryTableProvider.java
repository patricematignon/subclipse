...
/**
 * This class provides the table and it's required components for a file's
...
public class HistoryTableProvider {
    ...
    class HistorySorter extends ViewerSorter {
        ...
        /**
		 * Compares two log entries, sorting first by the main column of this
		 * sorter, then by subsequent columns, depending on the column sort
		 * order.
		 */
        ...
        int compareColumnValue(int columnNumber, ILogEntry e1, ILogEntry e2) {
            switch (columnNumber) {
                case COL_REVISION : /* revision */
                    return (
                        e1.getRevision().getNumber() < e2.getRevision().getNumber()
                            ? -1
                            : (e1.getRevision() == e2.getRevision() ? 0 : 1));
                case COL_DATE : /* date */
    ...
    /**
	 * Create a TableViewer that can be used to display a list of ILogEntry
	 * instances. Ths method provides the labels and sorter but does not
	 * provide a content provider
	 * 
	 * @param parent
	 * @return TableViewer
	 */
    ...
    private SelectionListener getColumnListener(final TableViewer tableViewer) {
        /**
		 * This class handles selections of the column headers. Selection of
		 * the column header will cause resorting of the shown tasks using that
		 * column's sorter. Repeated selection of the header will toggle
        ...
        return new SelectionAdapter() {
			 ...
			 * sorter of that column to be the current tasklist sorter.
			 * Repeated presses on the same column header will toggle sorting
