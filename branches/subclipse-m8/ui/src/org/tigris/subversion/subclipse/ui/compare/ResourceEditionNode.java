				...
				try {
					SVNUIPlugin.runWithProgress(null, true /*cancelable*/, new IRunnableWithProgress() {
						public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
							...
							try {
								ISVNRemoteResource[] members = ((ISVNRemoteFolder)resource).members(monitor);
								children = new ResourceEditionNode[members.length];
								for (int i = 0; i < members.length; i++) {
									children[i] = new ResourceEditionNode((ISVNRemoteResource)members[i]);
								}
	...
	public InputStream getContents() throws CoreException {
		...
		try {
			...
			SVNUIPlugin.runWithProgress(null, true /*cancelable*/, new IRunnableWithProgress() {
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					try {
						if(resource instanceof ISVNRemoteFile)
						holder[0] = ((ISVNRemoteFile)resource).getStorage(monitor).getContents();
					} catch (TeamException e) {
						throw new InvocationTargetException(e);
